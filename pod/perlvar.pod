...
Some identifiers are are exempt from the effects of C<package>,
and are always forced to be in package C<main>; they are also exempt 
from C<strict 'vars'> errors:

=over 4 

=item - identifiers that begin with digits, like C<$1>,C<$2> - regex captures.

=item - identifiers that begin with punctuation characters

=item - C<$a> and C<$b> - used by C<sort>

=item - C<STDIN>, C<STDOUT>, and C<STDERR> - the global file handles.

=item - C<ENV> C<INC> C<ARGV> C<ARGVOUT> C<SIG>

=item - special C<${^_XYZ}> variables
...
rmdir, say, sin, split (for its second argument), sqrt, stat, study, uc, ucfirst,
unlink, unpack.
